...
failure you have, start at [diagnosing a failure](/moose-inference/troubleshooting/).
...
[detection](/moose-inference/get-started/choose-a-provider/#or-dont-choose) pick one that works.
...
[common failures](/moose-inference/troubleshooting/common-failures/#no-provider-is-available).
...
The package resolved and then failed: an ABI mismatch, a missing system library, or a Node version
the native build does not support.
...
[a grammar does not enforce](/moose-inference/local/choosing-a-model/#what-changes-under-a-grammar).
...
<Aside type="danger" title="A 429 does not look like a rate limit">
  ...
  [forces `human-review`](/moose-inference/judge/#the-exact-boundaries). Under concurrency a rate limit
...
[exec reference](/moose-inference/reference/exec/#execresult).

## See it for yourself

Every message above is provoked for real by
[`examples/diagnose-errors.mjs`](https://github.com/hawkeyexl/moose-inference/blob/main/examples/diagnose-errors.mjs),
